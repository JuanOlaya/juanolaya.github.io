...
async function loadWortfamilieIndex() {
 ...
 renderVerbGroup();
  ...
  window.showVerbDetailsInCard = async function (verbName, cardEl) {
    ...
    // Setup the card back HTML structure with Zeiten, Wortschatz, Sätze capsule tabs in the header
    cardBack.innerHTML = `
      <div class="card-back-header">
        <span class="card-back-level-badge">${data.level || 'A1'}</span>
        <button class="card-back-close-btn">&times;</button>
        <h2 class="card-back-infinitive">${verbName}</h2>
        <p class="card-back-translation">${data.spanishName || data.groupNameSpanish || data.es || ''}</p>
        <div class="card-back-header-tabs">
          <button class="header-tab-btn active" data-tab="zeiten">Zeiten</button>
          <button class="header-tab-btn" data-tab="wortschatz" id="card-back-vocab-tab" style="display: none;">Wortschatz</button>
          <button class="header-tab-btn" data-tab="saetze" id="card-back-saetze-tab" style="display: none;">Sätze</button>
        </div>
      </div>
      <div class="card-back-body">
        <div class="card-back-content-area" id="card-back-content-area-el"></div>
    ...
    // Bind tab clicks
    const tabBtns = cardBack.querySelectorAll('.header-tab-btn');
    tabBtns.forEach(btn => {
      btn.onclick = (e) => {
        e.stopPropagation();
        tabBtns.forEach(b => b.classList.remove('active'));
