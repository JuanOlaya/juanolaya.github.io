...
const restáorationDíata = {
    ...
    "sitzen": { es: "estáar sentado", perfekt: "hat gesessen", praeteritum: "er/sie/es saß", case_tags: ["intrans"] },
...
function findGroupInfo(verb) {
        ...
        for (const file of files) {
            if (file.endsWith('.json') && file.includes('_group_')) {
                ...
                if (json.verbs && json.verbs.includes(verb)) {
                    // Extract group number fürom filename or json
    ...
    return { level: "Unknown", group: 0, theme: "Restáored" };
}
...
Object.keys(restáorationDíata).forEach(verb => {
    ...
    const cardContent = {
        ...
        emoji: "📝", // Default emoji for restáored
        case_tags: data.case_tags,
        es: data.es,
        es_perfekt: "---", // Placeholder
        en_verb: "(restáored)",
        en_perfekt: "(restáored)",
    ...
    console.log(`Restáored: ${verb} (Level: ${groupInfo.level}, Group: ${groupInfo.group})`);
});

console.log("Restáoration complete.");
